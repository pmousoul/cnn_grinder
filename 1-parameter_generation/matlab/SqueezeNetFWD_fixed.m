% Forward path implementation of SqueezeNet v1.1 - Fixed-point version
clc;
clear all;
restoredefaultpath;
...
param_path = './Parameters/fixed/sqn/';
ires_path = './Inter-layer_Results/fixed/sqn/';
bin_path = './Binary_Files/fixed/sqn/';
...
two=  16;
w  =  8;
tfi=[ 0,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3];
tfo=[ 3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3,  3,  3,    3];
f  =[ 7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7,  7,  7,    7];
...
extract_params_iresults( ...
    ...
    model_path, 'deploy.prototxt', 'quantized_fixed.prototxt', param_path, ires_path, bin_path);
...
value_type_fm = 'int16'; % feature maps
...
% Reshape
reshape_rslt = layer_reshape( img, 3, 2, 32, 0, 'int16' );
conv_rslt = layer_conv_dfixed(reshape_rslt, weights, bias, 1, 1, 0, tfi(1), tfo(1), f(1), 'int16' );

conv_rslt = layer_relu(conv_rslt);
if (cmp)
    fprintf('\n');
    fprintf('SqueezeNet v1.1\n');
    fprintf('Fixed Point - FP version\n');
    fprintf('N x K x K x M (matlab) parameter organization\n');
    fprintf('Check intermediate results\n');
    fprintf('against Ristretto (fixed point)\n');
    fprintf('using 16-bit input\n');
    fprintf('and 8-bit parameters accuracy: \n\n');
...
% Pooling Layer 1
pool_rslt = layer_maxpool(conv_rslt, 3, 2, 0, 'int16');

if (cmp)
    load([ires_path '3_pool1.mat']);
    fprintf('Max error in maxpool1: %f\n', max( abs( double( data(:) ) - ( double( pool_rslt(:) ) * ( 2^(-tfo(1)) ) ) )));
end


% Fire Layer 2
load([param_path 'fire2_squeeze1x1_w.mat']); load([param_path 'fire2_squeeze1x1_b.mat']);
conv_rslt = layer_conv_dfixed(pool_rslt, weights, bias, 1, 1, 0, tfi(2), tfo(2), f(2), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire2_expand1x1_w.mat']); load([param_path 'fire2_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(3), tfo(3), f(3), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire2_expand3x3_w.mat']); load([param_path 'fire2_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(4), tfo(4), f(4), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);

conv_rslt = zeros (128, 56, 56, 'int16');
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(5), tfo(5), f(5), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire3_expand1x1_w.mat']); load([param_path 'fire3_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(6), tfo(6), f(6), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire3_expand3x3_w.mat']); load([param_path 'fire3_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(7), tfo(7), f(7), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);
...
% Pooling Layer 3
pool_rslt = layer_maxpool(conv_rslt, 3, 2, 1, 'int16');

if (cmp)
    load([ires_path '6_pool3.mat']);
    fprintf('Max error in maxpool3: %f\n', max( abs( double( data(:) ) - ( double( pool_rslt(:) ) * ( 2^(-tfo(7)) ) ) )));
end


% Fire Layer 4
load([param_path 'fire4_squeeze1x1_w.mat']); load([param_path 'fire4_squeeze1x1_b.mat']);
conv_rslt = layer_conv_dfixed(pool_rslt, weights, bias, 1, 1, 0, tfi(8), tfo(8), f(8), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire4_expand1x1_w.mat']); load([param_path 'fire4_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(9), tfo(9), f(9), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire4_expand3x3_w.mat']); load([param_path 'fire4_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(10), tfo(10), f(10), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);

conv_rslt = zeros (256, 28, 28, 'int16');
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(11), tfo(11), f(11), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire5_expand1x1_w.mat']); load([param_path 'fire5_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(12), tfo(12), f(12), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire5_expand3x3_w.mat']); load([param_path 'fire5_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(13), tfo(13), f(13), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);
...
if (cmp)
    ...
    fprintf('Max error in maxpool5: %f\n', max( abs( double( data(:) ) - ( double( pool_rslt(:) ) * ( 2^(-tfo(13)) ) ) )));
end


% Fire Layer 6
load([param_path 'fire6_squeeze1x1_w.mat']); load([param_path 'fire6_squeeze1x1_b.mat']);
conv_rslt = layer_conv_dfixed(pool_rslt, weights, bias, 1, 1, 0, tfi(14), tfo(14), f(14), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire6_expand1x1_w.mat']); load([param_path 'fire6_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(15), tfo(15), f(15), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire6_expand3x3_w.mat']); load([param_path 'fire6_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(16), tfo(16), f(16), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);

conv_rslt = zeros (384, 14, 14, 'int16');
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(17), tfo(17), f(17), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire7_expand1x1_w.mat']); load([param_path 'fire7_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(18), tfo(18), f(18), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire7_expand3x3_w.mat']); load([param_path 'fire7_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(19), tfo(19), f(19), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(20), tfo(20), f(20), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire8_expand1x1_w.mat']); load([param_path 'fire8_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(21), tfo(21), f(21), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire8_expand3x3_w.mat']); load([param_path 'fire8_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(22), tfo(22), f(22), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);

conv_rslt = zeros (512, 14, 14, 'int16');
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(23), tfo(23), f(23), 'int16' );
conv_rslt = layer_relu(conv_rslt);

load([param_path 'fire9_expand1x1_w.mat']); load([param_path 'fire9_expand1x1_b.mat']);
conv_rslt_1 = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(24), tfo(24), f(24), 'int16' );
conv_rslt_1 = layer_relu(conv_rslt_1);

load([param_path 'fire9_expand3x3_w.mat']); load([param_path 'fire9_expand3x3_b.mat']);
conv_rslt_2 = layer_conv_dfixed(conv_rslt, weights, bias, 3, 1, 1, tfi(25), tfo(25), f(25), 'int16' );
conv_rslt_2 = layer_relu(conv_rslt_2);
...
conv_rslt = layer_conv_dfixed(conv_rslt, weights, bias, 1, 1, 0, tfi(26), tfo(26), f(26), 'int16' );
conv_rslt = layer_relu(conv_rslt);

if (cmp)
    load([ires_path '14_conv10.mat']);
    fprintf('Max error in conv10: %f\n', max(abs( double( data(:) ) - ( double( conv_rslt(:) ) * ( 2^(-tfo(26)) ) ) )));
end
...
if (cmp)
    ...
    fprintf('Max error in avgpool10: %f\n', max( abs( double( data(:) ) - double( pool_rslt(:) ) ) ) );
end
...
if (cmp)
    fprintf('Max error in softmax: %f\n', max(abs( double( data(:) ) - double( soft_rslt(:) ) ) ) );
end
