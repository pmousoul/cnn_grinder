...
void preloadPixels(const coordinate_t y_center,
...
void preloadPixelsAndPrecalcCIoffset(
...
void macc2d(const data_t pixels[9], const data_t weights[9],
                               data_t& result) {
#pragma HLS inline
...
void processAllCHout(const channel_t ch_out,
                                        const channel_t ci,
...
void processInputChannel(const coordinate_t y,
...
data_t postprocess(const channel_t co,
                                      const weightaddr_t ci_offset) {
#pragma HLS INLINE
