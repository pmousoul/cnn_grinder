...
int main() {
  ...
  // Count total convolution time
	...
	stopwatch timer(1199999988); // for time measurements
#else
	stopwatch timer;
#endif
	double lat[30] = {0};
...
// We comment out printing to improve performance
...
L_LAYERS:
  for (int layer_id = 0; layer_id < net_CPU->num_layers; layer_id++) {
    ...
    fpga_top(layer, (data_t *)SHARED_DRAM, weights_offset, num_weights,
             input_offset);
    timer.stop();
    lat[layer_id] = timer.duration();
    tot_lat += lat[layer_id];

    #ifdef TRACE
      sleep(0.1);
    #endif

    // std::cout << "Layer latency (sec): " << lat << std::endl;
  ...
  std::cout << "TOTAL latency (sec): " << tot_lat << std::endl;
  std::cout << "Per layer latency (sec): " << std::endl;
  for (int layer_id = 0; layer_id < net_CPU->num_layers; layer_id++) {
    layer_t layer = net_CPU->layers[layer_id];
    std::cout << layer.name << ": " << lat[layer_id] << std::endl;
  }
