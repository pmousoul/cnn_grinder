	...
	int8_t* sw_fmap_dfp[4], int8_t* sw_params_dfp[SQN_LN], float* sw_fmap_flp_o, Largs_sqn& larg, float error_sz, bool* error_flag )
{
	...
	std::string implementation = "sw_sqn_dfp:";

	// Reshape the input from 227x227x3 to 113x113x32
	timer.start();
	reshape_input<int8_t>( sw_fmap_dfp[0], sw_fmap_dfp[1], 227, 227, 3, 32, 3, 2, 0 );
	timer.stop();
	...
	timer.start();
	sw_maxpool<int8_t>(
		sw_fmap_dfp[0], sw_fmap_dfp[1], larg.h_in[i], larg.w_in[i], larg.ch_out[i],
	...
	i++; ii++;
		...
		check_iresults<int8_t>(
			sw_fmap_dfp[1], SQN_FMAP_MAX_SZ, -ei[ii],
	...
	#ifdef USEDEBUG
		check_iresults<int8_t>(
			sw_fmap_dfp[0], SQN_FMAP_MAX_SZ, -ei[ii],
	...
	timer.start();
	sw_maxpool<int8_t>(
		sw_fmap_dfp[0], sw_fmap_dfp[2], larg.h_in[i], larg.w_in[i], larg.ch_out[i],
		larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	merge_channels<int8_t>(
		sw_fmap_dfp[2], sw_fmap_dfp[1], sw_fmap_dfp[0], larg.h_in[i+1], larg.w_in[i+1], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	merge_channels<int8_t>(
		sw_fmap_dfp[0], sw_fmap_dfp[2], sw_fmap_dfp[1], larg.h_in[i], larg.w_in[i], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	sw_maxpool<int8_t>(
		sw_fmap_dfp[1], sw_fmap_dfp[2], larg.h_in[i], larg.w_in[i], larg.ch_out[i],
		larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	sw_maxpool<int8_t>(
		sw_fmap_dfp[1], sw_fmap_dfp[0], larg.h_in[i], larg.w_in[i], larg.ch_out[i],
		larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	timer.start();
	merge_channels<int8_t>(
		sw_fmap_dfp[2], sw_fmap_dfp[0], sw_fmap_dfp[1], larg.h_in[i+1], larg.w_in[i+1], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	merge_channels<int8_t>(
		sw_fmap_dfp[0], sw_fmap_dfp[2], sw_fmap_dfp[3], larg.h_in[i], larg.w_in[i], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++;
	// C10_3
	...
	timer.start();
	merge_channels<int8_t>(
		sw_fmap_dfp[3], sw_fmap_dfp[1], sw_fmap_dfp[0], larg.h_in[i], larg.w_in[i], 2*larg.ch_out[i] , 1000-2*larg.ch_out[i], 2*larg.ch_out[i] - (1000-2*larg.ch_out[i]) );
	timer.stop();
	latency[y++] = timer.duration();

	// Convert C10 result to float
	for(int k=0; k < (larg.h_in[i]*larg.w_in[i]*1000); k++ )
		sw_fmap_flp[k] = (float) ( sw_fmap_dfp[0][k] ) * exp2(-eo[ii]);
