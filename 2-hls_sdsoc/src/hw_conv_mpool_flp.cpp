// HW CNN Convolution-Max-pooling implementation
	...
	param_t _bias[PAR_FACT][Q_CHO_MAX] )
{
	...
	L_LOAD_W_I: for ( uint8_t i = 0; i != PAR_FACT; i++ )
	{
		...
		L_LOAD_W_N: for ( uint32_t n = 0; n != Q_CHOxKxKxCHI/WB_PACK; n++ )
		{
			...
			L_LOAD_W_NN: for ( uint8_t nn = 0; nn != WB_PACK; nn++)
				_weights[i][ ( n*WB_PACK + nn )] = ppar.data[ nn ];
		}
	}
	...
	fmap_t linebuf_win[CHI_NUM], param_t _weights[CHI_NUM], acc_t *macc_buf )
{
	...
	L_MUL: for ( uint8_t n = 0; n != CHI_NUM; n++ )
	{
		...
		DO_PRAGMA ( HLS resource variable=tmp core=FMul_fulldsp )
		tmp =  itmp * wtmp;
		mul_buf[n] = tmp;
	}
	...
	acc_t maccp;
	DO_PRAGMA ( HLS resource variable=maccp core=FAddSub_fulldsp )
	maccp = win3[0] + win3[1];
	...
	fmap_t linebuf_win[CHI_NUM], param_t _weights[CHI_NUM], acc_t *macc_buf )
{
	...
	L_MUL: for ( uint8_t n = 0; n != CHI_NUM; n++ )
	{
		...
		fmap_t itmp = linebuf_win[ n ];
		param_t wtmp = _weights[ n ];
		DO_PRAGMA ( HLS resource variable=tmp core=FMul_nodsp )
		tmp =  itmp * wtmp;
		mul_buf[n] = tmp;
	}
	...
	acc_t maccp;
	DO_PRAGMA ( HLS resource variable=maccp core=FAddSub_nodsp )
	maccp = win3[0] + win3[1];
...
static void calc_ch_out(
	...
	param_t _weights[PAR_FACT][Q_CHOxKxKxCHI_MAX],
	param_t _bias[PAR_FACT][Q_CHO_MAX],
	fmap_t _fmaps_out[PAR_FACT][Q_CHO_MAX],
	uint16_t KxKxCHI, uint32_t Q_CHOxKxKxCHI )
{
	...
	L_READ_BIAS: for ( uint8_t i = 0; i != PAR_FACT; i++ )
	{
		DO_PRAGMA ( HLS pipeline II=1 )
		macc_buf[i] = static_cast <acc_t> ( _bias[ i ][ q_cho ] );
	}
	...
	L_Q_CHOxKxKxCHI: for ( uint16_t idx=0, idy=0; idx != Q_CHOxKxKxCHI; idx+=CHI_NUM, idy+=CHI_NUM )
	{
		...
		if ( ( idx + CHI_NUM ) == times_KxKxCHI )
		{
			...
			L_WRITE_MACC_RES: for ( uint8_t i = 0; i != PAR_FACT; i++ )
			{
				...
				_fmaps_out[ i ][ q_cho ] =  static_cast <fmap_t> ( macc_buf[ i ] );

				macc_buf[i] = static_cast <acc_t> ( _bias[ i ][ q_cho + 1 ] );
...
static void write_pix(
	fmap_t _fmaps_out[PAR_FACT][Q_CHO_MAX], uint16_t Q_CHO,
	fmap_t *out_pix )
{
	uint16_t idx = 0;
	...
	uint32_t Q_CHOxKxKxCHI, uint16_t Q_CHO, fmap_t *out_pix )
{
	...
	calc_ch_out(
		linebuf_win, _weights, _bias, _fmaps_out, KxKxCHI, Q_CHOxKxKxCHI );

	// Write back calculated channels
	write_pix( _fmaps_out, Q_CHO, out_pix );
}
...
// Max-pool function
// Floating-point implementation of the max-pool layer
// Zero-pading the input is done on the fly
static void hw_maxpool_flp(
...
void hw_conv_mpool_flp(
		...
		uint8_t use_relu, uint8_t ppad, uint8_t pkernel, uint8_t pstride )
{
		...
		uint8_t use_relu, uint8_t ppad, uint8_t pkernel, uint8_t pstride )
{
	...
	hw_conv_flp( fmap_in, param, fmap_out,
		h_in, w_in, ch_in,
		h_out, w_out, ch_out,
		pad, kernel, stride, use_relu );

	hw_maxpool_flp( fmap_out, pfmap_out,
