...
#include "sqj2_tb.hpp"
#include "hw_func_fip.hpp"


void hw_sqn_fip(
	p_fmap_t* hw_fmap_fip[4], p_param_t* hw_params_fip[SQN_LN], float* hw_fmap_flp_o, Largs_sqn& larg, float error_sz, bool* error_flag )
{
	...
	std::string implementation = "hw_sqn_fip:";

	// Reshape the input from 227x227x3 to 113x113x32
	timer.start();
	reshape_input<int16_t>( (int16_t*)hw_fmap_fip[0], (int16_t*)hw_fmap_fip[1], 227, 227, 3, 32, 3, 2, 0 );
	timer.stop();
	...
	timer.start();
	hw_conv_mpool_fip(
		hw_fmap_fip[1], hw_params_fip[i], hw_fmap_fip[0], larg.h_in[i], larg.w_in[i],
	...
	sleep(TRACE_TIME);
		...
		check_iresults<int16_t>(
			(int16_t*)hw_fmap_fip[0], SQN_FMAP_MAX_SZ, -ei[ii],
	...
	timer.start();
	hw_conv_mpool_fip(
		hw_fmap_fip[0], hw_params_fip[i], hw_fmap_fip[1], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 1, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	...
	timer.start();
	hw_conv_mpool_fip(
		hw_fmap_fip[1], hw_params_fip[i], hw_fmap_fip[2], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 1, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	...
	timer.start();
	hw_conv_mpool_fip(
		hw_fmap_fip[1], hw_params_fip[i], hw_fmap_fip[3], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 1, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	...
	merge_channels<int16_t>(
		(int16_t*)hw_fmap_fip[2], (int16_t*)hw_fmap_fip[3], (int16_t*)hw_fmap_fip[0], larg.h_in[i], larg.w_in[i], larg.ch_out[i] , larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	merge_channels<int16_t>(
		(int16_t*)hw_fmap_fip[2], (int16_t*)hw_fmap_fip[3], (int16_t*)hw_fmap_fip[0], larg.h_in[i+1], larg.w_in[i+1], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	timer.start();
	hw_conv_mpool_fip(
		hw_fmap_fip[0], hw_params_fip[i], hw_fmap_fip[2], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 1, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	timer.start();
	merge_channels<int16_t>(
		(int16_t*)hw_fmap_fip[1], (int16_t*)hw_fmap_fip[2], (int16_t*)hw_fmap_fip[3], larg.h_in[i], larg.w_in[i], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++;
	// C10_3
	...
	timer.start();
	merge_channels<int16_t>(
		(int16_t*)hw_fmap_fip[1], (int16_t*)hw_fmap_fip[2], (int16_t*)hw_fmap_fip[0], larg.h_in[i], larg.w_in[i], larg.ch_out[i], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	timer.start();
	merge_channels<int16_t>(
		(int16_t*)hw_fmap_fip[3], (int16_t*)hw_fmap_fip[0], (int16_t*)hw_fmap_fip[1], larg.h_in[i], larg.w_in[i], 2*larg.ch_out[i] , 1000-2*larg.ch_out[i], 2*larg.ch_out[i] - (1000-2*larg.ch_out[i]) );
	timer.stop();
	latency[y++] = timer.duration();
	...
	for(int k=0; k < (larg.h_in[i]*larg.w_in[i]*1000); k++ )
		hw_fmap_flp[k] = (float) ( hw_fmap_fip[1][k/FM_PACK].data[k%FM_PACK] ) * exp2( -eo[ii] );
	timer.stop();
	latency[y++] = timer.duration();
