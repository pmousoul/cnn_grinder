	...
	p_fmap_t* hw_fmap_dfp[4], p_param_t* hw_params_dfp[ZQN_LN], float* hw_fmap_flp_o, Largs_zqn& larg, float error_sz, bool* error_flag )
{
	...
	float* hw_fmap_flp = new float[ZQN_FMAP_MAX_SZ];
	...
	std::string implementation = "hw_zqn_dfp:";

	// Reshape the input from 256x256x3 to 128x128x32
	timer.start();
	reshape_input<int8_t>( (int8_t*)hw_fmap_dfp[0], (int8_t*)hw_fmap_dfp[1], 256, 256, 3, 32, 3, 2, 1 );
	timer.stop();
	...
	sleep(TRACE_TIME);
		...
		check_iresults<int8_t>(
			(int8_t*)hw_fmap_dfp[0], ZQN_FMAP_MAX_SZ, -ei[ii],
	...
	timer.start();
	hw_conv_mpool_dfp(
		hw_fmap_dfp[0], hw_params_dfp[i], hw_fmap_dfp[3], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 1, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	timer.start();
	merge_channels<int8_t>(
		(int8_t*)hw_fmap_dfp[2], (int8_t*)hw_fmap_dfp[3], (int8_t*)hw_fmap_dfp[1], larg.h_in[i+1], larg.w_in[i+1], larg.ch_out[i-1], larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	// F8E1
	...
	timer.start();
	merge_channels<int8_t>(
		(int8_t*)hw_fmap_dfp[3], (int8_t*)hw_fmap_dfp[0], (int8_t*)hw_fmap_dfp[1], larg.h_in[i], larg.w_in[i], larg.ch_out[i-1] , larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	timer.start();
	merge_channels<int8_t>(
		(int8_t*)hw_fmap_dfp[2], (int8_t*)hw_fmap_dfp[1], (int8_t*)hw_fmap_dfp[0], larg.h_in[i], larg.w_in[i], larg.ch_out[i-2] , larg.ch_out[i-2], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	i++; ii++;
	...
	hw_conv_mpool_dfp(
		hw_fmap_dfp[0], hw_params_dfp[i], hw_fmap_dfp[1], larg.h_in[i], larg.w_in[i],
		larg.ch_in[i], larg.ch_out[i], larg.pad[i], larg.kernel[i], larg.stride[i],
		ei[ii], eo[ii], ep[ii], 0, larg.ppad[i], larg.pkernel[i], larg.pstride[i] );
	timer.stop();
	latency[y++] = timer.duration();
	i++;
	// C10_12
	...
	timer.start();
	merge_channels<int8_t>(
		(int8_t*)hw_fmap_dfp[3], (int8_t*)hw_fmap_dfp[0], (int8_t*)hw_fmap_dfp[1], larg.h_in[i], larg.w_in[i], 2*larg.ch_out[i] , 2*larg.ch_out[i], 0 );
	timer.stop();
	latency[y++] = timer.duration();
	...
	timer.start();
	sw_avgpool_flp(
		hw_fmap_flp, hw_fmap_flp_o, larg.h_in[i], larg.w_in[i], 1024, 0, 8, 1 );
	timer.stop();
	latency[y++] = timer.duration();
	...
	// Print propabilities
	DEBUG( "ZynqNet fixed-point HW implementation - Top-5 results:" );
	DEBUG( "-------------------------------------------------------" );
#ifdef USEDEBUG
#if defined(__SIM__)
